        ...
        for(int i = 0; i < parts; i++){
            ...
            while(V_Nparts - V_n != 0.000000001){
                ...
                for(int j = 0; j < holes; j++){
                    if((l_cheese > table_values[j][1]) && (end_partCheese < table_values[j][3])){
                    ...
                    if((l_cheese < table_values[j][1]) && (end_partCheese > table_values[j][1]) && (end_partCheese < table_values[j][3])){
                        V_n -= Pi * pow((end_partCheese - table_values[j][1]), 2) * (table_values[j][0] - (end_partCheese - table_values[j][1])/3.0);
                    }
                    if((l_cheese < table_values[j][3]) && (end_partCheese > table_values[j][3]) && (l_cheese > table_values[j][1])){
